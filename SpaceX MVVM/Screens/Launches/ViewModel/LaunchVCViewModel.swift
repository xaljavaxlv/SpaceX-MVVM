...
final class LaunchVCViewModel: LaunchVCViewModelProtocol { // обязательно final? По умолчанию же сам ставится final под капотом если никто не наследуется
    ...
    private func prepareStrings() { // так норм или лучше разбить подготовку каждго стринга на отдельную функцию?
        ...
        for launch in launches {
            guard let dateRaw = launch.dateLocal else { return }
            let date = formatter.string(from: dateRaw)
            let name = launch.name ?? "Unknown"
            ...
            let launchString = LaunchStrings(name: name, date: date, imageName: imageName)
